    ...
    for i in range(len(img_list)):
        ...
        if (height != -1) and (width != -1):
            if isinstance(width, torch.Tensor):
                width = width.item()  # This assumes width is a single-element tensor
            if isinstance(height, torch.Tensor):
                height = height.item()  # This assumes height is a single-element tensor
            print(height,width)
            # Now width and height are integers, not tensors
