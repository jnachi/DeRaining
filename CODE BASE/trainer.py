...
def Pre_train(opt):
    ...
    def save_model(opt, epoch, iteration, len_dataset, generator):
        ...
        """
        if opt.save_mode == 'epoch':
            model_name = 'KPN_rainy_image_epoch%d_bs%d_prob100.pth' % (epoch, opt.train_batch_size)
        if opt.save_mode == 'iter':
            model_name = 'KPN_rainy_image_iter%d_bs%d_prob100.pth' % (iteration, opt.train_batch_size)
